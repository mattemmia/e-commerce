...
export default function Cart() {
  ...
  const sendToWhatsApp = async () => {
    ...
    // THIS MATCHES YOUR ADMIN.JSX EXACTLY
    const orderData = {
      customerName: form.name,
      customerPhone: form.phone,
      customerAddress: form.address,
      productName: cart.map(item => `${item.name} x${item.qty}`).join(', '),
      price: subtotal, // must be number for totalSales calculation
      items: cart,
      total: subtotal,
      status: 'processing', // lowercase to match StatusBadge
  ...
  if (cart.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center">
        <ShoppingBagIcon className="mx-auto h-16 w-16 text-zinc-300 dark:text-zinc-700" />
        <h2 className="mt-4 text-2xl font-bold text-zinc-900 dark:text-white">Your cart is empty</h2>
        <p className="mt-2 text-zinc-500 dark:text-zinc-400">Looks like you haven't added anything yet</p>
        <Link to="/" className="mt-6 inline-block rounded-xl bg-zinc-900 px-6 py-3 text-white font-semibold dark:bg-white dark:text-zinc-900">
          Continue Shopping
        </Link>
      </div>
    );
